    ...
    <div class="wrapper my-10 h-full flex-1 w-full max-w-[1280px] mx-auto">
      ...
      >
        ...
        >
          ...
          <ul v-if="expandedFolders.data.includes(categoryKey)" class="pl-8">
            <li v-for="link in categoryValue" :key="link.path">
              <nuxt-link
                class="flex items-baseline gap-2 text-primary-500 hover:text-emerald-500"
                :to="link.path"
              >
                <Icon
                  class="relative top-[4px] text-emerald-500"
